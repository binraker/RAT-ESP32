...
bool successful_syncToBrowser = false;
...
{ // Mdns services
  ...
  {
    LOG_ERR("mDNS host name: %s Failed", hostName);
  }
}
  ...
  if (strlen(ST_SSID) > 0)
  {
    ...
    if (strlen(ST_ip) > 1)
    {
      ...
      if (!_ip.fromString(ST_ip))
        LOG_ERR("Failed to parse %s", ST_ip);
...
bool getLocalNTP()
{
  // get current local time from NTP server and apply to ESP32
  ...
  if (getEpoch() > 10000)
  {
    ...
    struct tm now;
    getLocalTime(&now, 0);
    // CAUTION! Maintain same format between NTP and Browser, i.e, no space after ':'
    Serial.print("[NTP]:");
...
bool changeExtension(char *outName, const char *inName, const char *newExt)
{
  ...
  while (inNamePtr > 0 && inName[inNamePtr] != '.')
    inNamePtr--;
  inNamePtr++;
...
size_t isSubArray(uint8_t *haystack, uint8_t *needle, size_t hSize, size_t nSize)
{
  ...
  while (h < hSize && n < nSize)
  {
    ...
    if (haystack[h] == needle[n])
    {
      ...
      if (n == nSize)
        return h; // position of end of needle
    }
    else
    {
...
static void remote_log_free_telnet()
{
  ...
  {
    if (close(log_serv_sockfd) != 0)
      LOG_ERR("Cannot close the socket");
    log_serv_sockfd = -1;
...
static void remote_log_init_SD()
{
  ...
  if (log_remote_fp == NULL)
  {
    LOG_ERR("Failed to open SD log file %s", LOG_FILE_PATH);
  }
  else
  {
    LOG_INF("Ospened SD file for logging");
  }
}
...
#define NO_OF_SAMPLES 16 // ADC multisampling
  ...
  for (int j = 0; j < NO_OF_SAMPLES; j++)
    ADCsample += adc1_get_raw(BATT_PIN);
  ADCsample /= NO_OF_SAMPLES;
  // convert ADC averaged pin value to curve adjusted voltage in mV
  if (ADCsample > 0)
    ADCsample = esp_adc_cal_raw_to_voltage(ADCsample, adc_chars);
...
void setupADC()
{
  ...
  if (VOLTAGE_DIVIDER)
  {
    ...
    {
      LOG_INF("ADC characterised using eFuse Two Point Value");
    }
    else if (val_type == ESP_ADC_CAL_VAL_EFUSE_VREF)
    {
      LOG_INF("ADC characterised using eFuse Vref");
    }
    else
    {
      LOG_INF("ADC characterised using Default Vref");
    }
