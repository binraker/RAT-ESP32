/*
  Credits: 
  - Adopted from: https://github.com/jameszah/CameraWebServerRecorder
*/

#include "avi_recorder.h"

int framesize = 9;              // use FRAMESIZE_SVGA
int quality = 12;               // 10 - 12 is okay
int avi_length = 10;            // in seconds
int frame_interval = 100;       // for 10 frames per second
const int speed_up_factor = 1;  // DO NOT CHANGE!
...
// e.g framesize = 10 or 9 or 7 or 6 or 5..
...
char capturing_synced_Syntiant_AVI_FileName[50];
void set_AVI_FileName(const char *asChar_AVI_FileName){
  //LOG_INF("asChar_AVI_FileName: %s", asChar_AVI_FileName);
  // Copy the inputString to the global variable
  snprintf(capturing_synced_Syntiant_AVI_FileName, sizeof(capturing_synced_Syntiant_AVI_FileName)-1, "%s", asChar_AVI_FileName);
  LOG_INF("Proceeding to capture an AVI, %s ..", capturing_synced_Syntiant_AVI_FileName);
}
...
esp_err_t init_sdcard() {
  ...
  if (succ) {
    ...
    LOG_INF("SD_MMC Card Size: %lluMB", cardSize);
    ...
    */
    LOG_INF("Failed to mount SD card VFAT filesystem.");
    LOG_INF("Do you have an SD Card installed?");
    LOG_INF("Check pin 12 and 13, not grounded, or grounded with 10k resistors!");
  }
  return ESP_OK;
}
...
camera_fb_t *  get_good_jpeg() {
  ...
  do {
    ...
    if (!fb) {
      ...
      if (!foundffd9) {
        ...
        LOG_INF("Bad jpeg, Frame %d, Len = %d \n", frame_cnt, fblen);
  ...
  if (failures == 10) {
    ...
    LOG_INF("Decreasing quality due to frame failures %d -> %d", qual, qual + 5);
    delay(1000);
...
static void start_avi() {
  ...
  LOG_INF("Starting an avi.. ");

  avifile = SD_MMC.open(capturing_synced_Syntiant_AVI_FileName, "w");
  idxfile = SD_MMC.open("/idx.tmp", "w");

  if (avifile) {
    //Serial.printf("SD file opened: %s\n", capturing_synced_Syntiant_AVI_FileName);
    LOG_INF("SD file opened: %s", capturing_synced_Syntiant_AVI_FileName);
  }  else  {
    //Serial.println("Could not open file");
    LOG_INF("Could not open file: %s", capturing_synced_Syntiant_AVI_FileName);
  }
  ...
  if (idxfile)  {
    ...
    LOG_INF("Could not open file /idx.tmp");
  }
  ...
  */
  LOG_INF("Recording %u seconds.", avi_length);
...
/*
    - another_save_avi saves another frame to the avi file, updates index
...
static void another_save_avi(camera_fb_t * fb ) {
  ...
  if (err != fb_block_length) {
    ...
    */
    LOG_INF("Error on avi write: err = "); //LOG_INF(err);
    //LOG_INF(" len = "); //LOG_INF(fb_block_length);
  }
...
static void end_avi() {
  ...
  LOG_INF("End of avi - closing the files..");

  if (frame_cnt <  5 ) {
    //Serial.println("Recording has a problem, less than 5 frames, forget index\n");
    LOG_INF("Recording has a problem, less than 5 frames, forget index");
    idxfile.close();
    avifile.close();
    int xx = SD_MMC.remove("/idx.tmp");
    int yy = SD_MMC.remove(capturing_synced_Syntiant_AVI_FileName);
    ...
    */
    LOG_INF("*** Video recorded and saved ***");

    LOG_INF("Recorded %5d frames in %5d seconds", frame_cnt, elapsedms / 1000);
    LOG_INF("File size is %u bytes", movi_size + 12 * frame_cnt + 4);
    LOG_INF("Adjusted FPS is %5.2f", fRealFPS);
    LOG_INF("Max data rate is %lu bytes/s", max_bytes_per_sec);
    LOG_INF("Frame duration is %d us", us_per_frame);
    LOG_INF("Average frame length is %d bytes", uVideoLen / frame_cnt);

    LOG_INF("Writing the index, %d frames", frame_cnt);
    ...
    if (idxfile)  {
      ...
      LOG_INF("Could not open index file");
    }
...
void record_avi_video () {
  ...
  */
  LOG_INF("Total space: %lluMB", SD_MMC.totalBytes() / (1024 * 1024));
  LOG_INF("Used space: %lluMB", SD_MMC.usedBytes() / (1024 * 1024));
  ...
  while (1) {
    ...
    */
    ...
    ///////////////////  START A MOVIE  //////////////////
      ...
      /*
      Serial.printf("\nStarting the avi ... at %d\n", avi_start_time);
      Serial.printf("Framesize %d, quality %d, length %d seconds\n\n", framesize, quality, avi_length);
      */
      LOG_INF("Starting the avi ... at %d", avi_start_time);
      LOG_INF("Framesize %d, quality %d, length %d seconds", framesize, quality, avi_length);
    ...
    ///////////////////  END THE MOVIE //////////////////
      ...
      if (blinking) digitalWrite(33, HIGH); // turn light off
      ...
      */
      ...
      LOG_INF("End the avi at %d.  It was %d frames, %d ms at %.2f fps...", millis(), frame_cnt, avi_end_time, avi_end_time - avi_start_time, fps);
      LOG_INF("Total space: %lluMB", SD_MMC.totalBytes() / (1024 * 1024));
      LOG_INF("Used space: %lluMB", SD_MMC.usedBytes() / (1024 * 1024));
